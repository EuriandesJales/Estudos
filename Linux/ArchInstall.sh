...
# encoding: utf-8
# Versao 0.3
# Nao execute o  mesmo faz mudanças drasticas no seu hd
...
#read -p "Qual será a senha do Root?" ROOT_PASSWD
ROOT_PASSWD=senha

########## Variáveis Para Particionamento do Disco
# ATENÇÃO, este script apaga TODO o conteúdo do disco especificado em $HD.
HD=/dev/sda #faça a partição previamente que vai da tudo certo
HDHOME=/dev/sda #HD da partição Home
...
#modo interativo (faze de desenvolvimento)
#fdisk -l
#read -p "Qual HD o sistema deve ficar? " HD
#read -p "Qual HD deve ficar sua HOME(por padão usa o mesmo HD do Sistema): " HDHOME
#read -p "Tamanho que deseja para a raiz do sistema ROOT: " ROOT_SIZE
#ROOT_SIZE=$((ROOT_SIZE*1024))
#read -p "qual será o Tamanho do Swap? (em GB): " SWAP_SIZE
#SWAP_SIZE=$((SWAP_SIZE*1024))
#read -p "Tamanho da partição Home do Usuario: " HOME #renomei para HOME_SIZE
#HOME=$((HOME*1024))
...
EXTRA_PKGS='vim'
#zsh python3 vscode firefox libre office neofetch ipython
...
BOOT_START=1
BOOT_END=$(($BOOT_START+$BOOT_SIZE)) #tamanho definitivo da partição boot

SWAP_START=$BOOT_END
SWAP_END=$(($SWAP_START+$SWAP_SIZE)) #tamanho definitivo do swap adiciona espaço necessario pra carregar o bot

ROOT_START=$SWAP_END
ROOT_END=$(($ROOT_START+$ROOT_SIZE)) #adiciona o espaço do shwap ao espaço do root swap file

HOME_START=$ROOT_END #Pega resto de espaço para usar na Home (desabilitado)
...
#parted -s -- $HD mkpart primary $HOME_FS $HOME_START -0 1>/dev/null #home no mesmo hd do root usando o espaço restante
parted -s -- $HDHOME mkpart primary $HOME_FS $HOME -0 1>/dev/null
#
...
#mkfs.$BOOT_FS "$HD""1" -L Boot 1>/dev/null # versão Original
mkfs.$BOOT_FS "$HD""1" -L Boot 1>/dev/null
echo "Formatando partição root"
#mkfs.$ROOT_FS $HDHOME-L Root 1>/dev/null # versão Original
mkfs.$ROOT_FS "$HD""3" -L Root 1>/dev/null
echo "Formatando partição home"
#mkfs.$HOME_FS /dev/sda4 -L Home 1>/dev/null # versão Original
mkfs.$HOME_FS "$HDHOME""4"-L Home 1>/dev/null
#CODIGO ORIGINAL PRESUME QUE O HD É O /dev/sda
